...
const porteKey=x=>(x||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace("é","e");

async function init(){
 if(!sb){$("systemStatus").textContent="CONFIGURAÇÃO AUSENTE"; renderMessage("Configure config.js para conectar ao banco."); return;}
 ...
 if(pr.error){renderMessage("Perfil não encontrado. Execute o SQL e verifique o cadastro do usuário.");return;}
 profile=pr.data;
...
async function load(){
 const {data,error}=await sb.from("creatures").select("*").order("name");
 if(error){renderMessage("Falha ao consultar o banco: "+error.message);return;}
 allCreatures=data||[];
}
...
function switchSection(s){section=s;document.querySelectorAll(".nav").forEach(x=>x.classList.toggle("active",x.dataset.section===s));$("catalogToolbar").style.display=s==="manage"?"none":"flex";
 const info={catalog:["DATABASE / WILDLIFE","CATÁLOGO DE ESPÉCIES","Registros comuns e espécies públicas."],players:["DATABASE / PLAYERS","ESPÉCIES DOS PLAYERS","Fichas vinculadas aos personagens da campanha."],seasonal:["DATABASE / RESTRICTED","ARQUIVOS SAZONAIS","Somente o Mestre pode acessar estes registros."],manage:["MASTER CONTROL / CRUD","GERENCIAR CRIATURAS","Criar, editar, duplicar e excluir registros do banco."]}[s];
 $("sectionEyebrow").textContent=info[0];$("sectionTitle").textContent=info[1];$("sectionSubtitle").textContent=info[2];render();}
function visible(){
 ...
 const s=$("sort").value; arr=[...arr].sort((a,b)=>s==="level"?b.level-a.level:s==="hp"?b.hp-a.hp:a.name.localeCompare(b.name));
 return arr;
}
function render(){if(section==="manage"){renderManage();return;}const arr=visible();$("count").textContent=arr.length;$("contentGrid").innerHTML=arr.map(c=>card(c)).join("")||`<div class="empty">NENHUM REGISTRO ENCONTRADO.</div>`;document.querySelectorAll("[data-open]").forEach(x=>x.onclick=()=>openDetail(x.dataset.open));}
function card(c){const defense=10+Number(c.attrs?.agilidade||0);return `<article class="creature-card" data-open="${c.id}"><div class="record-code">${esc(c.category).toUpperCase()} // LV ${c.level}</div><div class="card-body"><div><h2>${esc(c.name)}</h2><p>${esc(c.species||"Espécie não catalogada")}</p><small>${esc(c.porte)} • ${esc(c.tipo||"—")} • ${esc(c.habitat||"—")}</small></div>${c.image_url?`<img src="${esc(c.image_url)}" alt="">`:"<div class=\"thumb-placeholder\">◈</div>"}</div><div class="stats-mini"><span>HP ${c.hp}</span><span>EN ${c.energia}</span><span>DEF ${defense}</span></div></article>`}
function openDetail(id){const c=allCreatures.find(x=>x.id===id);if(!c)return;const a=c.attrs||{};const attacks=Array.isArray(c.attacks)?c.attacks:[];const perks=c.perks||{};const specials=Array.isArray(c.specials)?c.specials:[];const loot=Array.isArray(c.loot)?c.loot:[];
...
function renderManage(){
 ...
 $("contentGrid").innerHTML=`<div class="manage-actions"><button class="btn primary" id="newCreature">+ NOVA CRIATURA</button><button class="btn" id="importSeed">IMPORTAR DADOS INICIAIS</button></div><div class="table-wrap"><table><thead><tr><th>Nome</th><th>Categoria</th><th>Porte</th><th>HP</th><th>Lv</th><th>Ações</th></tr></thead><tbody>${allCreatures.map(c=>`<tr><td>${esc(c.name)}</td><td>${esc(c.category)}</td><td>${esc(c.porte)}</td><td>${c.hp}</td><td>${c.level}</td><td><button class="btn tiny" data-edit="${c.id}">EDITAR</button> <button class="btn tiny danger" data-del="${c.id}">EXCLUIR</button></td></tr>`).join("")}</tbody></table></div>`;
...
function openEditor(c=null){
 ...
 <form id="creatureForm" class="editor">
 <div class="form-grid">
 ...
 <label>Player responsável<select name="owner_id"><option value="">— Não atribuído —</option>${profiles.filter(p=>p.role==="player").map(p=>`<option value="${p.id}" ${c?.owner_id===p.id?"selected":""}>${esc(p.username)}</option>`).join("")}</select></label>
 ...
 <label>Imagem (URL ou caminho)<input name="image_url" value="${esc(c?.image_url||"")}"></label>
 </div>
 ...
 <div class="form-grid">
 <label>ATAQUES <textarea name="attacks" placeholder="Mordida | Porte + Força">${jsonArrayText(c?.attacks)}</textarea></label>
 <label>PERÍCIAS <textarea name="perks" placeholder="Camuflagem | 2">${jsonPerksText(c?.perks)}</textarea></label>
 <label>PERÍCIAS ESPECIAIS <textarea name="specials" placeholder="Camuflagem II">${jsonArrayText(c?.specials)}</textarea></label>
 <label>LOOT <textarea name="loot" placeholder="Presa&#10;Couro">${jsonArrayText(c?.loot)}</textarea></label>
 </div>
 <label>Observações<textarea name="notes">${esc(c?.notes||"")}</textarea></label>
 <div class="editor-actions"><button type="submit" class="btn primary">SALVAR NO BANCO</button><button type="button" class="btn" id="cancelEdit">CANCELAR</button></div>
 <p id="editorMsg" class="message"></p></form>`;
 $("modal").classList.remove("hidden");$("cancelEdit").onclick=()=>$("modal").classList.add("hidden");
...
async function saveCreature(id,fd){
 ...
 const porte=fd.get("porte"), payload={name:fd.get("name"),species:fd.get("species"),level:+fd.get("level"),porte,porte_key:porteKey(porte),tipo:fd.get("tipo"),habitat:fd.get("habitat"),category:fd.get("category"),hp:+fd.get("hp"),energia:+fd.get("energia"),attrs:attrsObj,attacks:parseAttacks(fd.get("attacks")),perks:parsePerks(fd.get("perks")),specials:lines(fd.get("specials")),loot:lines(fd.get("loot")),image_url:fd.get("image_url"),notes:fd.get("notes"),owner_id:fd.get("owner_id")||null};
 const {error}=id?await sb.from("creatures").update(payload).eq("id",id):await sb.from("creatures").insert(payload);
 if(error){$("editorMsg").textContent="Erro: "+error.message;return;}
 await load();$("modal").classList.add("hidden");render();
}
async function deleteCreature(id){if(!confirm("Excluir este registro permanentemente?"))return;const {error}=await sb.from("creatures").delete().eq("id",id);if(error)alert(error.message);else{await load();renderManage();}}
async function importSeed(){
 const seeds=[...(window.CRX_SEED?.common||[])];
 for(const x of seeds){const {owner_username,...rest}=x;await sb.from("creatures").insert({...rest,category:"common"});}
 const playerSeeds=window.CRX_SEED?.players||[];
 for(const x of playerSeeds){
  const {owner_username,...rest}=x; let owner=null;
  if(owner_username){const {data}=await sb.from("profiles").select("id").eq("username",owner_username).maybeSingle();owner=data?.id||null;}
  await sb.from("creatures").insert({...rest,owner_id:owner,category:"player"});
 }
 await load();renderManage();alert("Importação concluída. Se já havia registros, eles foram duplicados.");
}
init();
